var counter = {
  ...
  //  after : what to do after countdown end (function)
    ...
    target.innerHTML = "";
    if (seconds >= 604800) { target.appendChild(gensquare("Weeks")); }
    ...
    if (seconds > 0) { target.appendChild(gensquare("Seconds")); }else{target.appendChild(gensquare("Closed"));}
  ...
  //  idx : target count down timer
    ...
    let weeks = Math.floor(secs / 604800); // 1 week = 60 secs * 60 mins * 24 hrs * 7 days
    secs -= weeks * 604800;    
    ...
    if (!(inst.closed !== undefined))
    {
      inst.seconds.innerHTML = secs;
    }else
    {
      inst.closed.style.display = 'none';
    }
    ...
    if (inst.weeks !== undefined) { inst.weeks.innerHTML = weeks; }
