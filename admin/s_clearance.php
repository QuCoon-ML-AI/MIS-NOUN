                ...
                <div class="innercont_stff_tabs" id="ans2" style="height:80%;">
                    <table id="gridz2" class="table table-condensed table-responsive" style="width:100%;">
                        ...
                        <tbody><?php
                            ...
                            if ($cdeptId_loc == 'PCC')
                            {
                                $sub_query = collect_fees_to_pay($_REQUEST["uvApplicationNo"], $cEduCtgId_loc, $cFacultyId_loc, 'PCC', 1, $cAcademicDesc_1, $cResidenceCountryId_loc);
                            }else if ($cdeptId_loc == 'CPE')
                            {
                                $sub_query = collect_fees_to_pay($_REQUEST["uvApplicationNo"], $cEduCtgId_loc, $cFacultyId_loc, 'CPE', 1, $cAcademicDesc_1, $cResidenceCountryId_loc);
                            }else
                            {
                                $sub_query = collect_fees_to_pay($_REQUEST["uvApplicationNo"], $cEduCtgId_loc, $cFacultyId_loc, 'All', 1, $cAcademicDesc_1, $cResidenceCountryId_loc);
                            }

                            $stmt_reuse = $mysqli->prepare("SELECT citem_cat_desc, b.fee_item_id, b.fee_item_desc vDesc, Amount, iItemID 
                            ...
                            AND b.fee_item_desc <> 'Final Clearance'
                            AND b.fee_item_desc <> 'Application Fee'
                            AND b.fee_item_desc <> 'Convocation Gown'
                            AND study_mode_ID = 'odl' 
                            AND (a.cdel = 'N' OR b.fee_item_desc = 'JAMB regularization fee')
                            AND c.cEduCtgId = '$cEduCtgId_loc' $sub_query
                            ORDER BY a.citem_cat, vDesc");
                            
                            $stmt_reuse->execute();
                            $stmt_reuse->store_result();
                            $stmt_reuse->bind_result($citem_cat_desc, $fee_item_id, $vDesc, $Amount, $iItemID);

                            $str = '';
                            while ($stmt_reuse->fetch())
                            {
                                if (substr($_REQUEST["uvApplicationNo"],3,2) < 24 && $vDesc == 'Acceptance Fee')
                                {
                                    continue;
                                }
                                ...
                                {
                                    $wrking_tab_loc = 's_tranxion_'.$value;

                                    $stmt_bmpf = $mysqli->prepare("SELECT tdate  
                                    FROM $wrking_tab_loc
                                    WHERE fee_item_id = $fee_item_id
                                    AND vMatricNo  = ?");
                                    $stmt_bmpf->bind_param("s", $_REQUEST["uvApplicationNo"]);                                        
                                    $stmt_bmpf->execute();
                                    $stmt_bmpf->store_result();
                                    $stmt_bmpf->bind_result($reg_tdate);
                                    $stmt_bmpf->fetch();

                                    if ($cEduCtgId_loc == 'PSZ')
                                    {
                                        if ($reg_tdate < '2022-12-19')
                                        {
                                            if ($vDesc == 'Caution Fee')
                                            {
                                                $Amount = 3500;
                                            }else  if ($vDesc == 'Identity Card')
                                            {
                                                $Amount = 500;
                                            }else  if ($vDesc == 'Matriculation Fee')
                                            {
                                                $Amount = 1500;
                                            }else  if ($vDesc == 'Orientation Fee')
                                            {
                                                $Amount = 1500;
                                            }else  if ($vDesc == 'Result Verification')
                                            {
                                                $Amount = 5000;
                                            }
                                        }
                                    }else if ($cEduCtgId_loc == 'PGX' || $cEduCtgId_loc == 'PGY')
                                    {
                                        if ($reg_tdate < '2022-12-19')
                                        {
                                            ...
                                            {
                                                $Amount = 10000;
                                            }else  if ($vDesc == 'Transcript verification')
                                            {
                                                $Amount = 0;
                                            }
                                        }
                                    ...
                                    if (!is_null($reg_tdate))
                                    {
                                        //echo $reg_tdate.','.$fee_item_id.'; ';
                                        break;
                                    }
                                }
                ...
                <div class="innercont_stff_tabs" id="ans3" style="height:80%;">
                    <table id="gridz3" class="table table-condensed table-responsive" style="width:100%;">
                        ...
                        <tbody><?php
                            ...
                            for ($x = 1; $x <= $tSemester_loc; $x++)
                            {
                                if (substr($_REQUEST["uvApplicationNo"],3,2) <= 23)
                                {
                                    if ($cEduCtgId_loc == 'PSZ' || $cEduCtgId_loc == 'PGX' || $cEduCtgId_loc == 'PGY')
                                    {
                                        $efac_fee = 4000;
                                        $ict_fee = 5000;
                                        $lib_fee = 3000;
                                ...
                                if ($cnt%2==0){$background_color='#dbe3dc';}else{$background_color='#FFFFFF';}?>
                                 <tr style="background-color:<?php echo $background_color;?>; font-weight:bold">
                                    <td style="text-align:right; width:8%; padding-right:2%"><?php echo ++$cnt;?></td>
                                    <td style="width:45%;">Semester <?php echo $x;?></td>
                                    <td style="text-align:right; width:13%; padding-right:2%"></td>
                                    <td style="text-align:right; width:13%; padding-right:2%"></td>
                                    <td style="text-align:right; width:13%; padding-right:2%"></td>
                                </tr><?php                                
                                ...
                                <tr style="background-color:<?php echo $background_color;?>;">
                                    <td style="text-align:right; width:8%; padding-right:2%"><?php echo ++$cnt;?></td>
                                    <td style="width:45%;">e-Facilitation
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%"><?php 
                                        echo '-'.number_format($efac_fee);$balance -= $efac_fee;?>
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%">--
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%"><?php echo number_format($balance);?></td>
                                ...
                                <tr style="background-color:<?php echo $background_color;?>;">
                                    <td style="text-align:right; width:8%; padding-right:2%"><?php echo ++$cnt;?></td>
                                    <td style="width:45%;">ICT Fee
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%"><?php 
                                        echo '-'.number_format($ict_fee);$balance -= $ict_fee;?>
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%">--
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%"><?php echo number_format($balance);?></td>
                                ...
                                <tr style="background-color:<?php echo $background_color;?>;">
                                    <td style="text-align:right; width:8%; padding-right:2%"><?php echo ++$cnt;?></td>
                                    <td style="width:45%;">Library Fee
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%"><?php 
                                        echo '-'.number_format($lib_fee);$balance -= $lib_fee;?>
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%">--
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%"><?php echo number_format($balance);?></td>
                                ...
                                <tr style="background-color:<?php echo $background_color;?>;">
                                    <td style="text-align:right; width:8%; padding-right:2%"><?php echo ++$cnt;?></td>
                                    <td style="width:45%;">Registration Fee
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%"><?php 
                                        echo '-'.number_format($reg_fee);$balance -= $reg_fee;?>
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%">--
                                    </td>
                                    <td style="text-align:right; width:13%; padding-right:2%"><?php echo number_format($balance);?></td>
                                ...
                                //     $stmt_bmpf = $mysqli->prepare("SELECT DISTINCT tdate  
                                //     FROM $wrking_tab_loc
                                //     WHERE fee_item_id IN ('12','16','20','29')
                                //     AND vMatricNo  = ?");
                                //     $stmt_bmpf->bind_param("s", $_REQUEST["uvApplicationNo"]);                                        
                                //     $stmt_bmpf->execute();
                                //     $stmt_bmpf->store_result();
                            ...
                            /*if ($cdeptId_loc == 'PCC')
                            {
                                $sub_query = collect_fees_to_pay($_REQUEST["uvApplicationNo"], $cEduCtgId_loc, $cFacultyId_loc, 'PCC', 2, $cAcademicDesc_1, $cResidenceCountryId_loc);
                            }else if ($cdeptId_loc == 'CPE')
                            {
                                $sub_query = collect_fees_to_pay($_REQUEST["uvApplicationNo"], $cEduCtgId_loc, $cFacultyId_loc, 'CPE', 2, $cAcademicDesc_1, $cResidenceCountryId_loc);
                            }else
                            {
                                $sub_query = collect_fees_to_pay($_REQUEST["uvApplicationNo"], $cEduCtgId_loc, $cFacultyId_loc, 'All', 2, $cAcademicDesc_1, $cResidenceCountryId_loc);
                            }
                            ...
                            AND b.fee_item_desc <> 'Application Fee'
                            AND b.fee_item_desc <> 'Convocation Gown'
                            AND b.fee_item_desc <> 'Acceptance Fee'
                            ...
                            for ($x = 1; $x <= $tSemester_loc; $x++)
                            {
                                ...
                                $stmt_reuse->data_seek(0);
                                while ($stmt_reuse->fetch())
                                {
                                    if ($vDesc == 'Biometric and Proctoring Fee')
                                    {
                                        $stmt_bmpf = $mysqli->prepare("SELECT tdate  
                                        FROM $wrking_tab 
                                        WHERE tSemester = $x
                                    ...
                                    }else if (substr($_REQUEST["uvApplicationNo"],3,2) < 23)
                                    {
                                        if ($vDesc == 'e-Facilitation')
                                        {
                                            $Amount = 4000;
                                        }else  if ($vDesc == 'ICT Fee')
                                        {
                                            $Amount = 5000;
                                        }else  if ($vDesc == 'Library Fee')
                                        {
                                            $Amount = 3000;
                                        }else  if ($vDesc == 'Registration Fee')
                                        {
                                            $Amount = 6000;
                                        }
                                    }else
                                    {                                    
                                        foreach ($table as &$value)
                                        {
                                            ...
                                            if ($vDesc == 'Registration Fee')
                                            {
                                                // if ($cEduCtgId_loc == 'PSZ')
                                                // {
                                                //     if ($reg_tdate < '2023-01-01')
                                                //     {
                                                //         if ($Amount == 8500)
                                                //         {
                                                //             $Amount = 6000;
                                            ...
                                            // $stmt_bmpf = $mysqli->prepare("SELECT *  
                                            // FROM $wrking_tab 
                                            // WHERE fee_item_id = 72
                                            // AND tSemester = $x
                                            // AND vMatricNo  = ?");
                                            // $stmt_bmpf->bind_param("s", $_REQUEST["uvApplicationNo"]);
                    ...
                    <table id="gridz6" class="table table-condensed table-responsive" style="width:100%;">
                        ...
                        <tbody><?php
                            ...
                            foreach ($table as &$value)
                            {
                                ...
                                FROM $wrking_tab_loc a, fee_items b 
                                WHERE a.fee_item_id = b.fee_item_id
     ...
     if ($cResidenceCountryId_loc == 'NG')
     {
         ...
         }else if ($cEduCtgId_loc == 'PSZ')
         {
             if ($tSemester_loc <= 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'A1' /*or a.citem_cat = 'A4'*/)";
                //  }else
                //  {
         ...
         }else if ($cEduCtgId_loc == 'PGX')
         {
             if ($tSemester_loc <= 1)
             {
                 ...
                 //{
                     $sql_cat = " AND (a.citem_cat = 'B1' /*or a.citem_cat = 'B4'*/)";
                //  }else
                //  {
         ...
         {		
             if ($tSemester_loc <= 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'C1' /*or a.citem_cat = 'C4'*/)";
                //  }else
                //  {
         ...
         {		
             if ($tSemester_loc == 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'G1' /*or a.citem_cat = 'G3' or a.citem_cat = 'G4'*/)";
         ...
         {		
            ...
            //  {
                    $sql_cat = " AND (a.citem_cat = 'D1' /*or a.citem_cat = 'D3' or a.citem_cat = 'D4'*/)";
            //  }else if ($tSemester_loc%2 > 0)
            //  {
         ...
         }else if ($cEduCtgId_loc == 'ELZ')
         {
             if ($tSemester_loc == 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'E1' /*or a.citem_cat = 'E4'*/)";
                //  }else
                //  {
     ...
     }else
     {
         if ($cEduCtgId_loc == 'PSZ')
         {
             if ($tSemester_loc <= 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'H1' /*or a.citem_cat = 'H4'*/)";
                //  }else
                //  {
         ...
         }else if ($cEduCtgId_loc == 'PGX')
         {
             if ($tSemester_loc <= 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'I1' or a.citem_cat = 'I3' /*or a.citem_cat = 'I4'*/)";
                //  }else
                //  {
         ...
         }else if ($cEduCtgId_loc == 'PGY')
         {
             if ($tSemester_loc <= 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'J1' or a.citem_cat = 'J3' /*or a.citem_cat = 'J4'*/)";
                //  }else
                //  {
         ...
         }else if ($cEduCtgId_loc == 'PGZ')
         {
             if ($tSemester_loc <= 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'N1' or a.citem_cat = 'N3' /*or a.citem_cat = 'N4'*/)";
                //  }else
                //  {
         ...
         }else if ($cEduCtgId_loc == 'PRX')
         {
             if ($tSemester_loc <= 1)
             {
                ...
                //  {
                     $sql_cat = " AND (a.citem_cat = 'K1' or a.citem_cat = 'K3' /*or a.citem_cat = 'K4'*/)";
                //  }else
                //  {
             ...
             {
                 $sql_cat = " AND a.citem_cat = 'K5'";
             }
         }
     }
 
     $sql_feet_type = select_fee_srtucture_loc($vMatricNo, $cResidenceCountryId_loc, $cEduCtgId_loc);
 
     return $fac_dept_prog_spec_pay.$sql_cat.$sql_feet_type;
 }
 
 function select_fee_srtucture_loc($vMatricNo, $res_ctry, $cEduCtgId_loc)
{	
	$mysqli = link_connect_db();
