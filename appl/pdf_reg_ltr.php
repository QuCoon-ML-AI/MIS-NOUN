...
if ($stmt->num_rows <> 0)
{
    ...
    if ((isset($_REQUEST["user_cat"]) && ($_REQUEST["user_cat"] == '7' || $_REQUEST["user_cat"] == '6' || $_REQUEST["user_cat"] == '5' || $_REQUEST["user_cat"] == '3' || $_REQUEST["user_cat"] == '1')))
	{
        ...
        $pdf->SetFont('', '',12);
        $pdf->Cell(0,$row_height_25,'(Office of the Registrar)',$border, 1,'C');
        $pdf -> SetY(160);
        ...
        $pdf->SetFont('', 'B',11);
        $txt = 'Offer of Provisional Admission ['.$orgsetins["cAcademicDesc"].'/'.$orgsetins["tSemester"].' Session]';
        ...
        if ($cEduCtgId <> 'PSZ')
        {
            ...
            $txt2 = "Secretary, School of Postgraduate Studies";
        }
        
        $pdf->Image(BASE_FILE_NAME_FOR_IMG.$signat,59,705,100,20,'PNG');
